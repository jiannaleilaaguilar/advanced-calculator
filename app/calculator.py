...
class Calculator:
    """Facade orchestrating History, Memento, and Operation Factory."""
    def __init__(self):
    ...
    def undo(self):
        self.history_manager.history = self.caretaker.undo(
            self.history_manager.history
        )

    def redo(self):
        self.history_manager.history = self.caretaker.redo(
            self.history_manager.history
        )
...
def main():
    ...
    while True:  # pragma: no cover
        try:
            user_input = input(
                f"{Fore.YELLOW}calc> {Style.RESET_ALL}"
            ).strip().split()
            if not user_input:
                continue
            ...
            if cmd == 'exit':
                ...
                print_info("\n--- Arithmetic Commands ---")
                print_info("add, subtract, multiply, divide, power, root")
                print_info("modulus, int_divide, percent, abs_diff")
                print_info("\n--- Management Commands ---")
                print_info("history, clear, undo, redo, save, load, exit\n")
            elif cmd == 'history':
                if not calc.history_manager.history:
                    print_info("Calculation history is empty.")
                else:
                    for idx, item in enumerate(
                        calc.history_manager.history, 1
                    ):
                        print_info(
                            f"{idx}. {item.operation}({item.a}, {item.b}) "
                            f"= {item.result} [{item.timestamp}]"
                        )
            elif cmd == 'clear':
                ...
                print_success(
                    f"History saved to '{Config.HISTORY_FILE}'."
                )
            elif cmd == 'load':
                calc.history_manager.load_from_csv(Config.HISTORY_FILE)
                print_success(
                    f"History loaded from '{Config.HISTORY_FILE}'."
                )
            else:
                if len(user_input) != 3:
                    print_error(
                        "Invalid format: <operation> <number1> <number2>"
                    )
                    continue
